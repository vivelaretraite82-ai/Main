...
function ensureSortiesTarifColumn() {
  ...
  return new Promise((resolve) => {
    db.run(`ALTER TABLE sorties ADD COLUMN tarif TEXT`, (err) => {
      if (err && !(String(err.message || '').toLowerCase().includes('duplicate column'))) {
        console.error('Erreur ajout colonne tarif sur sorties:', err);
      }
      resolve();
    });
  });
...
function ensureUserProfileColumns() {
  ...
  return new Promise((resolve) => {
    db.run(`ALTER TABLE users ADD COLUMN adresse TEXT`, () => {
      db.run(`ALTER TABLE users ADD COLUMN naissance TEXT`, () => resolve());
    });
  });
}
  ...
  function finishInscription() {
    ...
    if (transporter) {
      transporter.sendMail({
        to: (process.env.ADMIN_EMAIL || 'lespetitesvirees@gmail.com'),
        from: process.env.FROM_EMAIL || 'no-reply@vivelaretraite.local',
        subject: 'Nouvelle inscription Les Petites Virées',
...
function getAdminUser(callback) {
  const primary = (process.env.ADMIN_EMAIL || 'lespetitesvirees@gmail.com').trim().toLowerCase();
  const secondary = (process.env.ADMIN_EMAIL2 || 'vivelaretraite82@gmail.com').trim().toLowerCase();
  db.get('SELECT id, email, prenom, nom, telephone FROM users WHERE email = ?', [primary], (e, row) => {
    if (row) return callback(null, row);
    db.get('SELECT id, email, prenom, nom, telephone FROM users WHERE email = ?', [secondary], callback);
  });
}
...
app.post('/auth/login', (req, res) => {
  ...
  if (!email || !password) return res.status(400).json({ error: 'email_password_required' });
  db.get('SELECT id, email, password_hash, prenom, nom, telephone, adresse, naissance FROM users WHERE email = ?', [email.trim().toLowerCase()], (err, row) => {
    if (err || !row) return res.status(401).json({ error: 'invalid_credentials' });
    const ok = bcrypt.compareSync(password, row.password_hash);
    if (!ok) return res.status(401).json({ error: 'invalid_credentials' });
    const token = signToken(row);
    res.json({ token, user: { id: row.id, email: row.email, prenom: row.prenom, nom: row.nom, telephone: row.telephone, adresse: row.adresse, naissance: row.naissance, isAdmin: isAdminEmail(row.email) } });
  });
});

app.get('/me', auth, (req, res) => {
  db.get('SELECT id, email, prenom, nom, telephone, adresse, naissance, created_at FROM users WHERE id = ?', [req.user.uid], (err, row) => {
...
app.put('/me', auth, (req, res) => {
  const { prenom, nom, telephone, adresse, naissance } = req.body || {};
  const fields = [];
  const values = [];
  if (typeof prenom !== 'undefined') { fields.push('prenom = ?'); values.push(prenom || null); }
  if (typeof nom !== 'undefined') { fields.push('nom = ?'); values.push(nom || null); }
  if (typeof telephone !== 'undefined') { fields.push('telephone = ?'); values.push(telephone || null); }
  if (typeof adresse !== 'undefined') { fields.push('adresse = ?'); values.push(adresse || null); }
  if (typeof naissance !== 'undefined') { fields.push('naissance = ?'); values.push(naissance || null); }
  if (fields.length === 0) return res.status(400).json({ error: 'no_fields' });
  const sql = `UPDATE users SET ${fields.join(', ')} WHERE id = ?`;
  values.push(req.user.uid);
  db.run(sql, values, (err) => {
    if (err) return res.status(500).json({ error: 'db_error' });
    db.get('SELECT id, email, prenom, nom, telephone, adresse, naissance FROM users WHERE id = ?', [req.user.uid], (e, row) => {
  ...
  db.get(q, [id, req.user.uid], (err, row) => {
    ...
    db.run(`UPDATE reservations SET status = 'cancelled' WHERE id = ? AND user_id = ?`, [id, req.user.uid], (delErr) => {
      ...
      if (transporter) {
        ...
        const lines = [
          'Annulation de réservation Les Petites Virées',
          '',
          'Membre : ' + fullName,
          'Email : ' + (row.email || ''),
...
app.get('/api/sorties', (req, res) => {
  db.all('SELECT id, titre, description, date_iso, lieu, categorie, image_path, tarif, created_at FROM sorties ORDER BY date_iso ASC, created_at DESC', (err, rows) => {
    if (err) return res.status(500).json({ error: 'db_error' });
    res.json(rows);
  });
});
...
app.post('/admin/api/sorties', auth, requireAdmin, (req, res) => {
  ...
  const stmt = db.prepare(`
    INSERT INTO sorties (titre, description, date_iso, lieu, categorie, image_path, tarif)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  stmt.run(
    String(titre).trim(),
...
function startServer(initialPort) {
  ...
  function attempt() {
    const server = app.listen(port, () => {
      console.log(`Serveur Les Petites Virées démarré sur http://localhost:${port}`);
    });
    server.on('error', (err) => {
